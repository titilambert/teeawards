...
class DeathsJob(Job):
    def __init__(self):
        ...
        """
        Job.__init__(self)
        results_db_name = 'results_deaths'
    ...
    def get_results(self):
        return self.load_results_from_cache()['deaths']
    ...
    def save_results_to_cache(self):
        ...
        if last_data is not None and last_data['deaths'] == self.results['deaths']:
    ...
    def process(self, player_name, gametype):
        ...
        if self.results is None:
            ...
            self.results['deaths'] = 0
            self.results['last_event_date'] = datetime(1,1,1,0,0,0)
        # Get new deaths
        if self.gametype:
            deaths = tee_db['kill'].find(spec={'$and': [
        ...
        # Set new deaths
        self.results['deaths'] += deaths.count()

        # Set last event date

        if deaths.count() > 0:
            self.results['last_event_date'] = deaths[0]['when']
